...
_SAAS_REGIONS = {"North America": 1.3, "EMEA": 1.1, "APAC": 0.9, "LATAM": 0.7}
...
def generate_saas() -> pd.DataFrame:
    ...
    for i, mo in enumerate(months):
        ...
        for plan, (arpu, base_cust, (g0, g1)) in _PLANS.items():
            for region, rmult in _SAAS_REGIONS.items():
                grow = g0 + (g1 - g0) * t
                ...
                rows.append({
                    "month": mo.isoformat(), "plan": plan, "region": region,
...
def generate_marketing() -> pd.DataFrame:
    ...
    for i, mo in enumerate(months):
        ...
        for ch, (base, cvr, aov, cps, (s0, s1)) in _CHANNELS.items():
            trend = s0 + (s1 - s0) * t
            sessions = int(base * trend * annual[mo.month] * float(rng.lognormal(0, 0.06)))
            conversions = int(sessions * cvr * float(rng.lognormal(0, 0.08)))
            revenue = round(conversions * aov * float(rng.lognormal(0, 0.05)), 2)
            ad_spend = round(sessions * cps * float(rng.lognormal(0, 0.05)), 2)
            rows.append({
                "month": mo.isoformat(), "channel": ch, "sessions": sessions,
