...
def compute_battery(profile: DatasetProfile, period: Optional[str] = None,
    ...
    """Compute the full grounded analysis bundle for the reporting period."""
    table, tcol = profile.table, profile.time_col
    measure = measure or pick_primary_measure(profile, db_path)
    period, prior = resolve_period(profile, period)

    period_total = _scalar(
        f'SELECT SUM("{measure}") FROM "{table}" WHERE {_month_filter(tcol, period)}', db_path)
    prior_total = _scalar(
        f'SELECT SUM("{measure}") FROM "{table}" WHERE {_month_filter(tcol, prior)}', db_path)
    delta = period_total - prior_total
    ...
    for dim in profile.dimensions:
        breakdown = run_select(
            ...
            f'WHERE {_month_filter(tcol, period)} GROUP BY "{dim}" ORDER BY total DESC',
            db_path=db_path, limit=1000)
        movers = run_select(
            f'WITH cur AS (SELECT "{dim}" v, SUM("{measure}") t FROM "{table}" '
            f'  WHERE {_month_filter(tcol, period)} GROUP BY v), '
            ...
            f'ORDER BY ABS(cur.t - COALESCE(prev.t,0)) DESC',
            db_path=db_path, limit=1000)
        dimensions[dim] = {
            ...
            "movers": AnalysisResult(key=f"movers_{dim}", title=f"{dim} movers ({prior} -> {period})",
                                     sql=movers.sql, columns=movers.columns, rows=movers.rows),
        }

    return {
        "table": table, "time_col": tcol, "primary_measure": measure,
...
def battery_to_markdown(b: dict) -> str:
    ...
    out = [f"PRIMARY MEASURE: {m}  |  PERIOD: {period} (vs prior {prior})",
           f"Total {m}: {b['period_total']:,.0f} vs {b['prior_total']:,.0f} prior "
           f"(change {b['delta']:+,.0f}, {pct})",
           "", f"### {b['trend'].title}", _ar_md(b["trend"])]
